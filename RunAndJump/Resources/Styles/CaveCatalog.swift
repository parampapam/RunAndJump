...
enum CaveCatalog {
    ...
    static let catalog = StyleCatalog(
        ...
        terrain: TerrainNames(
            ...
            // Узкая каменная планка со скруглёнными торцами — той же формы,
            // что у луга, и по той же причине: `PlatformSkin` рисует спрайт в
            // целый тайл и вешает его от верха площадки вниз, поэтому арт
            // обязан занимать только верхнюю часть кадра. Цельный блок
            // (`cave_platform_1` / `_2` в атласе) на этом месте свисал бы на
            // целый тайл ниже того, по чему игрок ходит.
