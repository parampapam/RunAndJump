...
enum EnemyKind: CaseIterable, Equatable {
    ...
    /// Оружие врага. Стреляет только снайпер — остальным `nil`, и это же
    /// отличает стрелка от обычного врага для сцены. Новый стреляющий вид —
    /// это ещё один case здесь, ни узел, ни сцена не меняются.
    ///
    /// Параметры подобраны так, чтобы снайпер был опасен, но честен: он видит
    /// примерно на экран вперёд, полсекунды целится, а снаряд летит быстрее
    /// игрока (250 пт/с), но не мгновенно.
    var weapon: EnemyWeapon? {
        ...
        case .sniper:
            return EnemyWeapon(
                ...
                cooldown: 1.6,
                projectileSpeed: 300,
                // Больше зоны видимости: выстрел с предельной дистанции должен
                // долетать, даже если игрок отступает.
                projectileRangeInTiles: 9
            )
        case .crab, .imp, .plant, .wasp:
            return nil
        }
    }
