...
enum PickupKind: Equatable {
    ...
    /// Остаётся ли награда поднятой после гибели игрока.
    ///
    /// Правило следует за тем, что переносится через смерть: очки за монеты
    /// сохраняются — значит, и сами монеты не возвращаются, иначе их можно было
    /// бы собирать повторно. Здоровье, наоборот, восстанавливается до
    /// стартового, поэтому аптечка возвращается на сцену: она — часть
    /// сложности участка, а не накопленный результат.
    var staysCollectedAfterDeath: Bool {
        switch self {
