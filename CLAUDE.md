...
- New hazard type (кислота, шипы) → add a case to `HazardKind` with its damage / interval plus its colours in the private extension in `Hazard.swift`; the scene and the builder already handle any kind. Place hazards with `HazardDescriptor(kind:rect:)` — прямоугольник задаёт и вид, и зону урона.
