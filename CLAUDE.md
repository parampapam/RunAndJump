...
- **`GameProgress` / `GameProgressRules`** — всё, что переживает пересоздание `GameScene`: индекс уровня, накопленный бонус, активная точка восстановления (`activeCheckpointIndex`) и уже поднятые награды (`collectedPickupIndices`). Два перехода: `playerDied` (очки уходят в перенос, уровень и точка остаются) и `levelCompleted` (индекс +1, точка и список наград обнуляются — они у каждого уровня свои).
