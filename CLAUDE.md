...
- **`LevelValidation`** — чистые проверки каталога стиля и уровня против него. Замена компилятору: открытое имя опечатку не ловит. Политика разная по категориям и это решение, а не мелочь: пустая **механическая роль** (плитка платформы) — уровень не собрать; **незнакомая декорация** — в DEBUG `assertionFailure`, в релизе пропуск, терять уровень из-за лишнего цветочка нельзя. Слои фона необязательны (у пещеры нет холмов, у луга — стены), поэтому компилятор больше не ловит уровень, выложивший фон чужим сегментом, — ловит `backgroundSegmentWithoutTexture`. Пустой сегмент (`HorizonSegment.isBlank`) от пропавшего слоя отличается тем, что не рисует ничего **по замыслу**.
...
- Новый вид фона → добавить случай в `HorizonSegment` / `SkySegment` и поле под него в `BackgroundNames` (слой ресурсов знает, чем сегмент нарисован, модель — только какой он). Поля, а не словарь по сегментам: новый сегмент **обязан** сломать сборку каждого каталога, а не тихо остаться ненарисованным. Узор фона уровня правится в его `BackgroundDescriptor`: чередование сегментов, ширина сегмента и высота линии горизонта. Ширина сегмента гряды не обязана совпадать с пропорцией картинки — у холмов и гор левый и правый край однотонные, растягивается только заливка. **У стены интерьера это не так**: она кроет экран целиком, однотонных краёв у неё нет, поэтому ширину берут по пропорции картинки, а сама картинка обязана тайлиться по горизонтали (проверяет `StyleAssetsTests`). **Узор держат коротким** (порядка четырёх сегментов): список длиннее полосы показывает только начало, а хвост не увидят никогда, и сколько влезет — зависит от размера экрана. Каждый слой фона — отдельный **Image Set**, а не кадр в атласе: слои рисуются по одному, каждый со своим `zPosition`, и паковать их вместе незачем. Новая картинка должна иметь тот же фоновый цвет, что и заливка её стиля (`bg_fill_day` `#B7CAF4` у луга, `bg_fill_cave` `#3A2D25` у пещеры, `bg_fill_castle` `#282A2E` у замка), иначе её прямоугольник станет виден.
