...
- **Озёра воды и лавы**: `HazardKind` (модель) знает урон и паузу между ударами, `Hazard` (узел) — только выкладывает озеро плитками жидкости из атласа `Hazards` (два кадра, все плитки анимируются в такт) и несёт тело-триггер. Плитки полупрозрачные и рисуются **поверх** игрока (`ZPosition.hazard`), а спрайт вошедшего в озеро игрока смещается вниз до полутайла (`Player.setSubmersion`, только `anchorPoint` — тело остаётся на опоре): выходит, что игрок утоплен по пояс и просматривается сквозь толщу. Глубину даёт чистая `HazardImmersion` — доля ширины игрока над зоной, поэтому на кромке он опускается и всплывает плавно, а не проваливается рывком по касанию тел. Под жидкостью лежит глухое «дно» цветом неба (`ZPosition.hazardPit`, `ScenePalette.sky`) — иначе сквозь неё просвечивал бы грунт с травой. У тела `collisionBitMask = none`: сквозь озеро проходят. Сцена держит список зон, в которых игрок сейчас находится (`didBegin`/`didEnd`), и каждый кадр зовёт общий `applyDamage(_:recovery:)` — тот же путь, что для врагов и снарядов, только окно неуязвимости берётся из вида зоны. Поэтому стоящего в лаве жжёт чаще, чем в воде, и оба урона не складываются с ударом врага.
...
- New hazard type (кислота, шипы) → add a case to `HazardKind` with its damage / interval, its frames in `AnimationFrames.Hazard` / `AnimationDuration.Hazard`, и непрозрачность в приватном расширении в `Hazard.swift`; the scene and the builder already handle any kind. Place hazards with `HazardDescriptor(kind:rect:)` — прямоугольник задаёт и вид, и зону урона, размер лучше в целых тайлах (озеро выкладывается плитками).
