...
struct GameRulesTests {
    ...
    @Test func enemyHitReducesHealthByOne() {
        let state = PlayerState(health: 5, bonusPoints: 10, locomotionMode: .normal)
    ...
    @Test func healthPickupIncreasesHealth() {
        let state = PlayerState(health: 3, bonusPoints: 0, locomotionMode: .normal)
    ...
    @Test func reachedPortalDoesNotChangeState() {
        let state = PlayerState(health: 3, bonusPoints: 7, locomotionMode: .normal)
        let new = GameRules.apply(.reachedPortal, to: state)
        #expect(new == state)
    }

    @Test func deathDetection() {
        #expect(GameRules.isDead(PlayerState(health: 0, bonusPoints: 5, locomotionMode: .normal)) == true)
        #expect(GameRules.isDead(PlayerState(health: 1, bonusPoints: 0, locomotionMode: .normal)) == false)
        #expect(GameRules.isDead(PlayerState(health: -1, bonusPoints: 0, locomotionMode: .normal)) == true)
    }
